...
BASE_URL = os.environ.get("BUTTERFLY_NOTEBOOK_URL", "")
DATA_DIR = os.path.join(os.path.dirname(__file__), "allotinus_data")
...
def query_species(species: str, prompt_override: str | None = None) -> str | None:
    ...
    question = prompt_override or (
        f"{species} - share the underside ID keys, including wing length/size, "
        "tail info, upperside id keys"
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:")
        print("  Single species:  python query_allotinus.py 'allotinus unicolor'")
        print("  List from file:  python query_allotinus.py --file allotinus_species_list.txt")
        print("  Status:          python query_allotinus.py --status")
        sys.exit(0)
    ...
    if sys.argv[1] == "--file":
        if len(sys.argv) < 3:
            print("Provide a file path: --file allotinus_species_list.txt")
            sys.exit(1)
