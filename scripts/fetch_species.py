#!/usr/bin/env python3
"""
Fetch Malaysian Allotinus species from iNaturalist and write data/species.json.
...
session.headers.update({"User-Agent": "Allotinus-ID/1.0 (github.com/chancsc/Allotinus_id)"})


def get_allotinus_taxon_id() -> int:
    print("Resolving Allotinus genus taxon ID…")
    r = session.get(f"{INAT_BASE}/taxa", params={"q": "Allotinus", "rank": "genus", "per_page": 20})
    r.raise_for_status()
    for taxon in r.json()["results"]:
        if taxon["name"] == "Allotinus":
            print(f"  Found: Allotinus = taxon_id {taxon['id']}")
            return taxon["id"]
    raise RuntimeError("Could not find Allotinus genus in iNaturalist taxa search")
...
def main():
    taxon_id = get_allotinus_taxon_id()
