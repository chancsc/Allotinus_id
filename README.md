...
A mobile-friendly web app for identifying common Malaysian *Allotinus* butterfly species from field photographs.

Live: [chancsc.github.io/Allotinus_id](https://chancsc.github.io/Allotinus_id)
...
```json
{
  "type": "result",
  "id": "r_unicolor",
  "name": "Allotinus unicolor unicolor",
  "common_name": "Common Playboy",
  "note": "Diagnostic note visible in the field.",
...
Fetches *Allotinus* species observed in Malaysia from the iNaturalist API and writes `data/species.json`.
...
| +1 per escape-hatch step | Path uses an escape-hatch choice listed in `ESCAPE_HATCHES` (`js/path-utils.js`) — routes an out-of-group species back into the main key |
| +100 contradiction | Path's starting branch contradicts the result's identification note (e.g. a tailed/tailless mismatch) |
...
| `[ESC]` | Canonical path uses an escape-hatch choice. Indicates a tree routing bug. |
